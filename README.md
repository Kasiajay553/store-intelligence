...
* `event_type` (TEXT) - e.g., `entry`, `exit`, `zone_entered`, `zone_exited`, `queue_completed`, `queue_abandoned`
* `track_id` (INTEGER) - Unique visitor ID generated by tracker
* `store_id` (TEXT) - Store code (e.g., `ST1008`)
* `camera_id` (TEXT) - Camera identifier (e.g., `CAM3`)
* `zone_id` (TEXT) - Layout zone code (e.g., `PURPLLE_MUM_1008_Z01`)
* `event_time` (TEXT) - ISO8601 string timestamp
* `payload` (TEXT) - Full JSON string representing the Pydantic model dump
* **Constraints:** `UNIQUE(event_type, track_id, store_id, event_time) ON CONFLICT IGNORE` (forces strict deduplication)
